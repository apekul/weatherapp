...
const DayForecast = ({ data }) => {
  ...
  return (
    <div className="bg-slate-800 rounded-2xl flex row-start-2 col-start-3 row-end-4 px-5 flex-col">
      <p className="text-slate-100 font-bold my-2">4 DAY FORECAST</p>
      <div className="flex flex-col justify-around h-full pb-2 gap-2 sm:gap-0">
        {sortDay().map((v, i) => (
          <div key={i} id={i} className="bg-slate-900 rounded-2xl py-2 px-5">
